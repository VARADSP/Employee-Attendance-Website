...
{
	$u = $_REQUEST['holidayname'];
	$e = $_REQUEST['holidaydate'];

	$ins = "insert into holidays(holidayname,holidaydate) values('$u','$e')";
...
{
	$d = $_REQUEST['del_id'];
	$del = "delete from holidays where id = '$d'";
	$ex = $con->query($del);
	header('location:holidays.php');
...
{
  $d = $_REQUEST['edit_id'];
  $del = "select * from holidays where id = '$edit_id'";
  $id = $edit_id;


$ex = $con->query($del);
$rw = $ex->fetch_object();
$holidayname=$rw->holidayname;
$holidaydate=$rw->holidaydate;
                    ...
                    while($rw = $ex->fetch_object())
                    {
                        ?>
										<tr class="holiday-upcoming">
											...
											<td><?php echo $rw->holidayname; ?></td>
											<td><?php echo $rw->holidaydate; ?></td>
											<td><?php

                      $originalDate = $rw->holidaydate;
$newDate = date("Y-d-m", strtotime($originalDate));
$date = str_replace('/', '-', $newDate);
$timestamp = strtotime($date);$day = date('l', $timestamp);
echo $day;
											...
											<td class="text-right">
												<div class="dropdown">
													...
													<ul class="dropdown-menu pull-right">
														...
														<li><a href="holidays.php?del_id=<?php echo $rw->id; ?>"  title="Delete"><i class="fa fa-trash-o m-r-5"></i> Delete</a></li>
													</ul>
												</div>
											</td>
										</tr>
						...
						<div class="modal-body">
							<form method="post">
								<div class="form-group">
									...
									<input class="form-control" required="" type="text" name="holidayname">
								</div>
								<div class="form-group">
								...
								<div class="m-t-20 text-center">
									<button type="submit" name="createholiday" class="btn btn-primary">Create Holiday</button>
								</div>
							</form>
						</div>
					</div>
